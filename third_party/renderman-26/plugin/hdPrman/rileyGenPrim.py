...
def PathCamelCaseToSnakeCase(path):
    s = "_".join(CamelCaseToSnakeCase(name) for name in path.split("/"))
    return s.replace(".", "_")
...
def ToComment(text, indent=0):
    ...
    for paragraph in re.split(r"\n\s*\n", text, re.DOTALL):
        for line in textwrap.wrap(" ".join(paragraph.split()), width=75 - indent):
            result += indent * " " + "// " + line + "\n"
        result += indent * " " + "//" + "\n"
...
def ApplyUnderlayToMembers(members):
    ...
    return [
        (name, type_name, global_member_opt_dict | opt_dict)
        for name, type_name, opt_dict in members
        if name != "ALL_MEMBERS"
    ]
...
def ProcessEntry(dstDir, **args):
    ...
    library_name = fields.setdefault(
        "LIBRARY_NAME", CapitalizeFirstLetter(library_path.split("/")[-1])
    )
    ...
    baseName = fields.setdefault(
        "FILE_NAME", UncapitalizeFirstLetter(schema_name) + "Prim"
    )
