...
def _compareFiles(installedFiles, generatedFiles, configuration):
    ...
    if len(installedFiles) != len(generatedFiles):
        ...
        if generatedNames - installedNames:
            exit(
                "*** Unknown files generated, please add them to the list of "
    ...
    for i in range(0, len(installedFiles)):
        ...
        with open(installedFiles[i], "r") as installedFile, open(
            generatedFiles[i], "r"
        ) as generatedFile:
...
def _copyGeneratedFiles(installedFiles, generatedFiles, diffs):
    ...
    for baseName, generatedFile, installedFile in zip(
        baseNames, generatedFiles, installedFiles
    ):
...
def _runBisonAndFlexCommands(configuration):
    ...
    flexCommand = lambda index: (
        [flexExecutable] + flexFlags(base) + [flexFiles[index]]
    )
...
def _parseArguments():
    ...
    parser.add_argument(
        "--srcDir", required=False, default=getcwd(), help="The source directory."
    )
    parser.add_argument(
        "--bison",
        required=True,
    ...
    parser.add_argument(
        "--flex", required=True, help="The location of the flex executable to be used."
    )
    parser.add_argument(
        "--validate",
        action="store_true",
    ...
    parser.add_argument(
        "--bases", nargs="+", help="Base file identifiers used for generation."
    )
...
def _getConfiguration():
    ...
    if not arguments.bases:
        ...
        if not bases:
            exit(
                "*** Unable to find source files for parser. Ensure that they "
...
if __name__ == "__main__":
    ...
    installedFiles = [join(configuration[SRC_DIR], basename(f)) for f in generatedFiles]
